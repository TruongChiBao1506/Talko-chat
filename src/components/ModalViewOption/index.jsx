...
function ModalViewOption({ isModalVisible, onCancel, data, onShowDetail }) {
  ...
  const handleCheckboxChange = (values) => {
    ...
    let options = newOptions.map((optionEle) => {
      ...
      if (flag) {
        let optionSearchTemp = {
          ...optionEle,
          userIds: [...optionEle.userIds],
        };
      ...
      <div className="modal-view-option">
        ...
        {(getMumberVotes() && getMumberVotes()) > 0 && (
          <p className="overview-text" onClick={handleShowDetail}>
            ...
            } người tham gia ${getMumberVotes()} lượt bình chọn `}
            <CaretRightOutlined />
          </p>
        )}
                ...
                <div className="modal-view-option_checkbox">
                  <Checkbox value={ele.name} checked={true}>
                    <div className="vote-message_item">
                      ...
                      <div
                        className="vote-message_progress"
                        style={{
                              ...
                              ({ getFieldValue }) => ({
                                validator(_, value) {
                                  ...
                                  getFieldValue("options").forEach((ele) => {
                                    if (
                                      ele &&
                                      ele.name.toLowerCase() === tempValue
                                    ) {
                                      count += 1;
                                    }
                                  });

                                  const checkDuplicate = infoVote.options.find(
                                    (ele) =>
                                      ele.name.toLowerCase() === tempValue
                                  );
                          ...
                          >
                            <Input
                              ...
                              placeholder={`Lựa chọn ${
                                infoVote && infoVote.options.length + index + 1
                              }`}
                              style={{ width: "100%" }}
                              suffix={
                                fields.length > 0 ? (
